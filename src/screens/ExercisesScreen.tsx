...
const ExercisesScreen: React.FC = () => {
  ...
  const handleCreate = async () => {
    ...
    if (isNaN(Number(expPerUnit)) || Number(expPerUnit) <= 0)
          ...
          <View style={[styles.card, { borderLeftColor: accentForCat(item.category) }]}>
            ...
            <View style={styles.tags}>
              <View style={styles.tag}>
                <Text style={styles.tagTxt}>+{item.exp_per_unit} EXP/{item.unit_label}</Text>
              </View>
              ...
              <View style={styles.tag}><Text style={styles.tagTxt}>{item.unit_label}</Text></View>
            </View>
          </View>
        )}
      />
        ...
        <KeyboardAvoidingView style={styles.overlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={styles.sheet}>
            ...
            <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.chips}>
                {UNIT_TYPES.map(u => (
                  <TouchableOpacity key={u.value}
                    ...
                    <Text style={[styles.chipTxt, unitType === u.value && styles.chipTxtOn]}>
                      {u.label}
                    </Text>
              ...
              {/* EXP per unit */}
              <SystemInput
                label={`EXP per ${UNIT_TYPES.find(u => u.value === unitType)?.suffix ?? 'unit'}`}
                value={expPerUnit}
              ...
              <Text style={styles.expHint}>
                Example: {expPerUnit || '2'} EXP per {UNIT_TYPES.find(u => u.value === unitType)?.suffix ?? 'unit'}.
                Doing 10 {UNIT_TYPES.find(u => u.value === unitType)?.suffix ?? 'units'} = {(Number(expPerUnit || 0) * 10).toFixed(0)} EXP.
              </Text>
              ...
              <SystemInput label="Stat +Value" value={statReward} onChangeText={setStatRew} keyboardType="numeric" />
