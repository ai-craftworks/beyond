 ...
 * =================================
 * Exercise library: list, create, delete.
 */
...
import { COLORS, EXERCISE_CATEGORIES, STATS } from '../constants/game';
...
const ExercisesScreen: React.FC = () => {
  ...
  const [expReward, setExpReward] = useState('20');
  ...
  const resetForm = () => { setName(''); setDesc(''); setExpReward('20'); setStatType('strength'); setStatRew('1'); setCategory('strength'); };

  const handleCreate = async () => {
    if (!name.trim())               return Alert.alert('System', 'Exercise name required.');
    if (isNaN(Number(expReward)))   return Alert.alert('System', 'Enter a valid EXP reward.');
    if (isNaN(Number(statReward)))  return Alert.alert('System', 'Enter a valid stat value.');
    setLoading(true);
    try {
      await createExercise({ name: name.trim(), description: desc.trim(), exp_reward: Number(expReward), stat_type: statType, stat_reward: Number(statReward), category });
      resetForm(); setModal(false); await load();
        ...
        renderItem={({ item }) => (
          <View style={[styles.card, { borderLeftColor: accentForCat(item.category) }]}>
            ...
            <View style={styles.tags}>
              <View style={styles.tag}><Text style={styles.tagTxt}>+{item.exp_reward} EXP</Text></View>
              ...
              <View style={styles.tag}><Text style={styles.tagTxt}>{item.category}</Text></View>
            </View>
          </View>
        )}
      />
            ...
            <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.chips}>
                {EXERCISE_CATEGORIES.map(c => (
                  <TouchableOpacity key={c.value} style={[styles.chip, category === c.value && styles.chipOn]}
              ...
              <View style={styles.chips}>
                {STATS.map(s => (
                  <TouchableOpacity key={s.key} style={[styles.chip, statType === s.key && styles.chipOn]}
              ...
              <View style={styles.row}>
                <SystemInput label="EXP Reward" value={expReward} onChangeText={setExpReward} keyboardType="numeric" style={styles.flex1} />
                <SystemInput label="Stat +Value"  value={statReward} onChangeText={setStatRew}  keyboardType="numeric" style={styles.flex1} />
              </View>

              <View style={styles.row}>
                <SystemButton title="Cancel" variant="ghost" style={styles.flex1} onPress={() => { resetForm(); setModal(false); }} />
                <SystemButton title="Create" style={styles.flex1} onPress={handleCreate} loading={loading} />
              </View>
            </ScrollView>
          </View>
...
const styles = StyleSheet.create({
  ...
  sheet:    { backgroundColor: COLORS.bgPanel, borderTopLeftRadius: 20, borderTopRightRadius: 20, borderTopWidth: 1, borderColor: COLORS.accentCyan, padding: 20, maxHeight: '92%' },
